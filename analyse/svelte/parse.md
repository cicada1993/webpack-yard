# svelte/compiler compile方法中调用parse方法分析
```
export default function parse(
	template: string,
	options: ParserOptions = {}
): Ast {
    // 这里template就是源码 创建Parser
...
export class Parser {
	...
	constructor(template: string, options: ParserOptions) {
		...
		// 这里fragment是个接收Parser对象的函数
		let state: ParserState = fragment;

		// 识别svelte模板中的内容 < 、{
		// < 代表着 script、style 及 html标签 使用tag函数处理
		// { 代表着 {/if}, {/each}, {/await} or {/key} 使用mustache函数处理
